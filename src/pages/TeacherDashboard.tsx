...
const CARDS: CardDef[] = [
  ...
  { emoji: '📄', title: '智能组卷', desc: '按题型分布从题库智能组卷，不足 AI 补充', path: '/teacher/exam-builder', accent: 'violet' },
  {
    emoji: '📖',
    title: '讲义制作',
    desc: '四种模式含自定义拖拽模板，封面/目录/页眉页脚导出',
...
export default function TeacherDashboard() {
  ...
  const [usage, setUsage] = useState({ handout: 0, book: 0 })
  ...
  const cards = useMemo(
    () => (profile?.role === 'admin' ? [...CARDS, ADMIN_CARD] : CARDS),
    [profile?.role],
  )
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map((c) => {
            const count = c.usageKey ? usage[c.usageKey] : 0
            return (
              <button
                key={c.path}
                type="button"
