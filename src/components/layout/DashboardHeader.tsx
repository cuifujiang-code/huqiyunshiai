...
interface Props {
  ...
  backTo?: string
  backLabel?: string
  /** 传入后在标题栏下方显示功能导航（出题 / 诊断 / 教育规划） */
  featureNavRole?: 'teacher' | 'student'
}

export default function DashboardHeader({ title, backTo, backLabel = '返回', featureNavRole }: Props) {
  ...
  return (
    <>
      ...
      {featureNavRole && <FeatureNav role={featureNavRole} />}
    </>
  )
}
