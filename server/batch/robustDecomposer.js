...
/**
 * 稳健拆题（不入库）：DeepSeek 拆题 → 标准化 → 公式/图片占位符替换
 * 供上传拆题、预览等场景复用，与批量拆题同一套 prompt 与 normalizer。
 *
 * @param {string} text 试卷文本（含【公式】、[图片占位符] 等）
 * @param {object} [meta] subject, grade, formulaImages, images, startSort, _formulaIdx, _imageIdx
 * @returns {Promise<object[]>}
 */
export async function decomposeTextToQuestions(text, meta = {}) {
  const chunkText = String(text ?? '').trim()
  if (!chunkText) return []
  ...
  const taskMeta = {
    ...
    formulaImages: meta.formulaImages || meta.formula_images || [],
    images: meta.images || meta.extracted_images || [],
  ...
  if (decomposeResult.skippedFragment) {
    console.log('[robustDecomposer] decomposeTextToQuestions 跳过噪声片段', {
      textLength: chunkText.length,
    })
    return []
  }

  const { rawArray } = decomposeResult
  const startSort = Number.isFinite(Number(meta.startSort)) ? Number(meta.startSort) : 0
  const { valid: normalizedRaw } = normalizeQuestionsBatch(rawArray, taskMeta, startSort)
  const normalizedQuestions = filterCompleteQuestions(normalizedRaw)

  meta._formulaIdx = taskMeta._formulaIdx
  meta._imageIdx = taskMeta._imageIdx

  console.log('[robustDecomposer] decomposeTextToQuestions 完成', {
    rawCount: rawArray.length,
    validCount: normalizedQuestions.length,
    formulaIdx: taskMeta._formulaIdx,
    imageIdx: taskMeta._imageIdx,
  })
