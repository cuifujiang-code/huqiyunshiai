...
COMMENT ON TABLE public.student_invite_codes IS '学生分享给家长的邀请码';
COMMENT ON TABLE public.student_parent_bindings IS '学生与家长绑定关系（多对多）';
...
CREATE POLICY "deny_direct_invite_codes"
  ON public.student_invite_codes FOR ALL USING (false) WITH CHECK (false);

DROP POLICY IF EXISTS "deny_direct_parent_bindings" ON public.student_parent_bindings;
CREATE POLICY "deny_direct_parent_bindings"
  ON public.student_parent_bindings FOR ALL USING (false) WITH CHECK (false);
